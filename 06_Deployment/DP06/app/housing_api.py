...
inference_executor = ThreadPoolExecutor(max_workers=4)  # 스레드풀 생성
...
async def startup():
    ...
    predictor = HousingPredictor(MODEL_PATH, PREPROCESS_PATH)  # HousingPredictor 인스턴스 생성
...
async def predict_housing(request: HousingRequest):
    ...
    # 요청 데이터를 딕셔너리로 변환
    features = request.model_dump()  # Pydantic 모델 → dict (v1의 경우 request.dict())

    try:
        # 추론 (별도 스레드에서 실행 — Day 3 패턴)
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor( # run_in_executor 사용
